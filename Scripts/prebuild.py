...
if os.path.exists("Scripts//config.json"):
...
try:
    ...
    json_ = open("Scripts//config.json", "r")
    path_details = {}
    ...
    data = ["sdp_file_path", "osa_file_path", "v_ecu_name", "container_file_path", "veos_build_tool", "output_file" ,
            ...
            "osa_author", "zip_filename", "artifact_path","dap_arxml","module","jfrog_repo","ecu_name","ecu_sprint_number"]
    ...
    vecu_name = path_details.get("ecu_name")
    repo = path_details.get("jfrog_repo")
    #ecu_variant = path_details.get("ecu_variant")
    ecu_sprint_number = path_details.get("ecu_sprint_number")


    #url = url + str(build_number)+"/"+str(zipfile_name)+".zip"
    url = repo+"/"+pipeline+"/"+branch+"/"+vecu_name+"/"+ecu_sprint_number+"/"+str(build_number)+"/"+str(zipfile_name)+".zip"
